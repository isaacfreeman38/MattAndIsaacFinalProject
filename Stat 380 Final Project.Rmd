...
GatheredIncome <-
  TableWYears %>% 
    ...
    filter(State == "Pennsylvania" | State == "West Virginia" | State == "Hawaii" | State == "Kansas")

GatheredIncome %>%
  #filter(State == "Pennsylvania") %>%
  ggplot(aes(x = year, y = median, color = State, group = as.factor(State))) + geom_line() + geom_point() + ylab("Median Income") + xlab("Year") +
...
graph1 <- 
  ...
  ggplot(aes(y = OverallRank, x = EPRank)) + 
  geom_point() + 
...
graph2 <- 
  ...
  ggplot(aes(y = OverallRank, x = WERank)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = 0) + 
  ylab("Overall Happiness Ranking") + 
...
graph3 <- 
  ...
  ggplot(aes(y = OverallRank, x = CERank)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = 0) + 
  ylab("Overall Happiness Ranking") + 
